...
export function ShuffleButton({ shuffle, onToggle }) {
  return React.createElement(
    ...
    React.createElement(Shuffle, { size: 16, "aria-hidden": true }),
  );
}
...
export function MusicSeekBar({ current, time, onSeek }) {
  ...
  return React.createElement(
    ...
    React.createElement(
      "span",
      { className: "music-seek-time" },
      `${formatTime(shown)} / ${formatTime(current.duration)}`,
    ),
  );
}
...
export function renderControls(state, handlers) {
  ...
  return React.createElement(
    ...
    React.createElement(ShuffleButton, {
      shuffle: !!state?.shuffle,
      onToggle: handlers.toggleShuffle,
    }),
    TransportButton({
      title: "Previous",
      icon: SkipBack,
