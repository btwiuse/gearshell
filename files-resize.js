...
function startSidebarResize(ctx, event) {
  const { stackedLayout, panelRef, sidebarResizeRef } = ctx;
...
function resizeSidebar(ctx, event) {
  const { sidebarResizeRef, setSidebarWidth, setSidebarHeight } = ctx;
...
function stopSidebarResize(ctx, event) {
  const { sidebarResizeRef } = ctx;
  const resize = sidebarResizeRef.current;
  if (!resize || resize.pointerId !== event.pointerId) return;
...
function resizeSidebarBy(ctx, delta) {
  const { stackedLayout, panelRef, setSidebarWidth, setSidebarHeight } = ctx;
...
export function useFilesSidebarResize({ stackedLayout, panelRef }) {
  ...
  return {
    sidebarWidth,
    sidebarHeight,
    startSidebarResize: (event) => startSidebarResize(ctx, event),
    resizeSidebar: (event) => resizeSidebar(ctx, event),
    stopSidebarResize: (event) => stopSidebarResize(ctx, event),
    resizeSidebarBy: (delta) => resizeSidebarBy(ctx, delta),
  };
}
