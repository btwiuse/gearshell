...
# to end via the gctl CLI: create a headless sub-task, poll its output,
# and use the result. Verified in the real browser (2026-08-29):
#
#   [agent] 1/3 create headless sub-task via gctl
#   [agent]    create -> {"ok":true,"panelId":"workspace-task-9",...}
#   [agent] 2/3 poll gctl tasks.output '[9]'
...
# parsing; gctl takes a JSON-array args string, so a scalar like the
...
echo "[agent] 1/3 create headless sub-task via gctl"
res=$(gctl tasks.create '[{"name":"demo-sub","cmd":"echo result=$((6*7))","term":false}]')
...
echo "[agent] 2/3 poll gctl tasks.output '[$panel]'"
out=""
for ((i = 0; i < 20; i++)); do
  raw=$(gctl tasks.output "[$panel]" 2>/dev/null || true)
  rest=${raw#*'"output":"'}
  out=${rest%%'"'*}
