...
function PipingSsh({ pipingServerUrl, username, defaultSshPassword, agentForwarding, onEnd, onConnected, onReconnecting, onSavePassword, isActive = true }) {
  ...
  useEffect(() => {
    ...
    const external = globalThis.GearShell?.terminal?.external;
    let externalInput = null;
    const externalReady = external
    ...
    if (external) {
      ...
      GearShell.on('terminal.external.data', onData);
      GearShell.on('terminal.external.resize', onResize);
      GearShell.on('terminal.external.close', onClose);
      externalEvents.push(['terminal.external.data', onData], ['terminal.external.resize', onResize], ['terminal.external.close', onClose]);
    }
    ...
    const doConnect = async () => {
      ...
      try {
        ...
        await remote.doSsh(
          ...
          Comlink.proxy({
            termWrite(data) {
              if (externalSessionRef.current) external.write(externalSessionRef.current, data).catch(() => {});
  ...
  return html`
    <div ref=${wrapperRef} style=${{ flex: 1, overflow: 'hidden', position: 'relative' }}>
      ...
      <div ref=${termRef}
        aria-hidden="true"
        style=${{ position: 'fixed', left: '-10000px', top: '-10000px', width: '1px', height: '1px', overflow: 'hidden', opacity: 0, pointerEvents: 'none' }}
      ></div>
    </div>
  `;
}
