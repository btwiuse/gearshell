...
export function TerminalPresetIconPicker({ value, onChange }) {
  ...
  const [recents, setRecents] = useState(() => {
    try {
      ...
      return Array.isArray(parsed)
        ? parsed.filter((id) => ICON_BY_ID[id]).slice(0, RECENT_LIMIT)
        : [];
    } catch {
      return [];
    }
  ...
  const filtered = ALL_OPTIONS.filter((option) =>
    matchesQuery(option) && cat.match(option.id)
  );
    ...
    open && React.createElement(
      ...
      React.createElement(
        ...
        React.createElement("input", {
          type: "search",
          value: query,
          placeholder: `Search ${ALL_OPTIONS.length} icons…`,
          ...
          React.createElement(
            ...
            recentOptions.map((option) => {
              ...
              return React.createElement("button", {
                key: option.id,
                type: "button",
                className: `terminal-profile-icon-option${
                  isSelected ? " selected" : ""
                }`,
