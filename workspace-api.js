...
import { WORKSPACE_TASK_STATUS_EVENT } from "./app-constants.js?v=20260828.106";
import {
  drainEvents,
  emit,
  eventBuffer,
...
import { openApi } from "./workspace-open-api.js?v=20260828.132";
import { configApi } from "./workspace-config-api.js?v=20260828.132";
import {
  runHeadlessTask,
  tasksApi,
} from "./workspace-tasks-api.js?v=20260828.132";
import { agentsApi } from "./workspace-agents-api.js?v=20260828.1";
import { musicApi } from "./music-engine.js?v=20260829.11";
import { terminalApi } from "./workspace-terminal-api.js?v=20260829.152";
import {
  applyW9yMod,
  installedModStatus,
  listInstalledMods,
  refreshW9yRegistry,
  removeW9yMod,
} from "./app-w9y-registry.js?v=20260830.38";
import {
  gcWorkspaceTasks,
  markAgentTaskStatus,
} from "./workspace-task-registry.js?v=20260828.132";
import { ensureGearShellBinds, GEAR_BIND } from "./gear-bind.js?v=20260828.132";
...
const api = {
  ...
  // apply/remove orchestrate headless `w9y mod apply/remove` tasks and
  // fire-and-forget from the sync jsfs surface; results arrive as
  // w9y.changed events (events.drain). list/status read the in-memory
  // mirror of the registry; refresh re-reads the file.
  w9y: {
    list: safe(() => ({ ok: true, packages: listInstalledMods() })),
    status: safe((id) => ({ ok: true, ...installedModStatus(id) })),
    refresh: safe(() => {
      refreshW9yRegistry();
      return { ok: true, note: "registry refresh started" };
    }),
    apply: safe((id, version) => {
      applyW9yMod(id, version);
      return {
        ok: true,
        id,
        note: `w9y mod apply ${id}${version ? `@${version}` : "@latest"} started`,
      };
    }),
    remove: safe((id) => {
      removeW9yMod(id);
      return { ok: true, id, note: `w9y mod remove ${id} started` };
    }),
  },
